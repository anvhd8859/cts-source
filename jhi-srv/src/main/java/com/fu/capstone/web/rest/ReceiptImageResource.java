...
public class ReceiptImageResource {
	 ...
	 *
	 * @param receiptImageDTO
	 *            the receiptImageDTO to create
	 * @return the ResponseEntity with status 201 (Created) and with body the
	 *         new receiptImageDTO, or with status 400 (Bad Request) if the
	 *         receiptImage has already an ID
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	@PostMapping("/receipt-images/{id}")
	@Timed
	public ResponseEntity<ReceiptImageDTO> createReceiptImage(@PathVariable Long id,
			@RequestBody MultipartFile data) throws Exception {
		ReceiptImageDTO receiptImageDTO = receiptImageService.store(id, data);
		return ResponseEntity.ok()
				.headers(HeaderUtil.createEntityCreationAlert(ENTITY_NAME, receiptImageDTO.getId().toString()))
				.body(receiptImageDTO);
	}
	 ...
	 *
	 * @param receiptImageDTO
	 *            the receiptImageDTO to update
	 * @return the ResponseEntity with status 200 (OK) and with body the updated
	 *         receiptImageDTO, or with status 400 (Bad Request) if the
	 *         receiptImageDTO is not valid, or with status 500 (Internal Server
	 *         Error) if the receiptImageDTO couldn't be updated
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	@PutMapping("/receipt-images")
	@Timed
	public ResponseEntity<ReceiptImageDTO> updateReceiptImage(@RequestBody ReceiptImageDTO receiptImageDTO)
			throws URISyntaxException {
	 ...
	 *
	 * @param id
	 *            the id of the receiptImageDTO to retrieve
	 * @return the ResponseEntity with status 200 (OK) and with body the
	 *         receiptImageDTO, or with status 404 (Not Found)
	 */
	 ...
	 *
	 * @param id
	 *            the id of the receiptImageDTO to delete
	...
	@GetMapping("/receipt-images/note/{id}")
	@Timed
	public ResponseEntity<ReceiptImageDTO> getReceiptImageByReceiptNote(@PathVariable Long id) {
		log.debug("REST request to get ReceiptImage : {}", id);
		ReceiptImageDTO receiptImageDTO = receiptImageService.getReceiptImageByReceiptNote(id);
