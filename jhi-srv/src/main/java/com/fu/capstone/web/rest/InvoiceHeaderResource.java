...
public class InvoiceHeaderResource {
    ...
    @PostMapping("/invoice-headers/create-full-invoice")
    @Timed
    public ResponseEntity<InvoiceHeaderDTO> createInvoiceHeaderDetailPackage(@RequestBody InvoicePackageDetailDTO InvoicePackageDetailDTO) throws URISyntaxException {
        if (InvoicePackageDetailDTO.getHeader().getId() != null) {
            throw new BadRequestAlertException("A new invoiceHeader cannot already have an ID", ENTITY_NAME, "idexists");
        }
        InvoiceHeaderDTO result = invoiceHeaderService.createInvoiceHeaderDetailPackage(InvoicePackageDetailDTO);
        return ResponseEntity.created(new URI("/api/invoice-headers/create-full-invoice/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(ENTITY_NAME, result.getId().toString()))
            .body(result);
    }
}
