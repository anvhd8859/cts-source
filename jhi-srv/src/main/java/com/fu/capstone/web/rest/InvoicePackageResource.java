...
public class InvoicePackageResource {
    ...
    public ResponseEntity<List<InvoicePackageShipmentDTO>> getExportPackageByOfficeId(@RequestParam("id") Long id,
			@RequestParam("wid") Long wid ,@RequestParam("invNo") String invNo, @RequestParam("status") String status,
			@RequestParam("fromDate") String fromDate, @RequestParam("toDate") String toDate, Pageable pageable) {
    	Page<InvoicePackageShipmentDTO> page = invoicePackageService.getExportPackageByOfficeId(id, wid, invNo, status, fromDate, toDate,  pageable);
