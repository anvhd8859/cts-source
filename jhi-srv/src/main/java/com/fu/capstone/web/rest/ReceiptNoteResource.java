...
public class ReceiptNoteResource {
	 ...
	 *            the receiptNoteDTO to create
	 * @return the ResponseEntity with status 201 (Created) and with body the
	 *         new receiptNoteDTO, or with status 400 (Bad Request) if the
	 *         receiptNote has already an ID
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	@PostMapping("/receipt-notes")
	@Timed
	public ResponseEntity<ReceiptNoteDTO> createReceiptNote(@RequestBody ReceiptNoteDTO receiptNoteDTO)
			throws URISyntaxException {
	 ...
	 *
	 * @param receiptNoteDTO
	 *            the receiptNoteDTO to update
	 * @return the ResponseEntity with status 200 (OK) and with body the updated
	 *         receiptNoteDTO, or with status 400 (Bad Request) if the
	 *         receiptNoteDTO is not valid, or with status 500 (Internal Server
	 *         Error) if the receiptNoteDTO couldn't be updated
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	@PutMapping("/receipt-notes")
	@Timed
	public ResponseEntity<ReceiptNoteDTO> updateReceiptNote(@RequestBody ReceiptNoteDTO receiptNoteDTO)
			throws URISyntaxException {
	 ...
	 *
	 * @param id
	 *            the id of the receiptNoteDTO to retrieve
	 * @return the ResponseEntity with status 200 (OK) and with body the
	 *         receiptNoteDTO, or with status 404 (Not Found)
	 */
	 ...
	 *
	 * @param id
	 *            the id of the receiptNoteDTO to delete
	...
	public ResponseEntity<ReceiptNoteDTO> getReceiptNoteByShipmentId(@RequestParam("id") Long id) {
		...
		return new ResponseEntity<>(receiptNoteDTO, HttpStatus.OK);
	}

	@GetMapping("/receipt-notes/by-user")
	@Timed
	public ResponseEntity<List<ReceiptInvoiceDTO>> getAllReceiptInvoiceByUser(@RequestParam("id") Long id,
			Pageable pageable) {
	...
	@Timed
	public ResponseEntity<ReceiptNoteDTO> createReceiptNoteColectShipment(@RequestBody ReceiptDetailPackageDTO data)
			throws URISyntaxException {
	...
	@Timed
	public ResponseEntity<ReceiptNoteDTO> createReceiptNoteDeliveryShipment(@RequestBody ReceiptDetailPackageDTO data)
			throws URISyntaxException {
		log.debug("REST request to update ReceiptNote : {}", data);
		if (data.getReceipt().getId() != null) {
			throw new BadRequestAlertException("A new receiptNote cannot already have an ID", ENTITY_NAME, "idexists");
